...
class ProcessAssets {
	...
	constructor(config) {
		...
		if ("collection" in config) {
			if (typeof config.collection !== "string") {
				throw new TypeError(
					`config.collection must be a string, received ${typeof config.collection}`,
				);
			}
		...
		if (typeof config.inExtension !== "string") {
			throw new TypeError(
				`config.inExtension must be a string, received ${typeof config.inExtension}`,
			);
		}
		...
		if (typeof config.outDirectory !== "string") {
			throw new TypeError(
				`config.outDirectory must be a string, received ${typeof config.outDirectory}`,
			);
		}
		...
		if (typeof config.outExtension !== "string") {
			throw new TypeError(
				`config.outExtension must be a string, received ${typeof config.outExtension}`,
			);
		}
	...
	async processDirectory() {
		...
		try {
					...
					try {
						...
						if (this.production) {
							try {
								...
								throw new Error(
									`Failed to generate hash for file "${file}". ` + `Error: ${error.message}`,
								);
							}
						}
			...
			throw new Error(`ProcessAssets.processDirectory() failed: ${error.message}`);
		}
